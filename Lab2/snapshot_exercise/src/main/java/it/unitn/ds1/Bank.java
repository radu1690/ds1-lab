...
public class Bank extends AbstractActor {
  ...
  private void captureState() {
    ...
    // COMPLETED 1: save current balance and enter snapshot mode
    capturedBalance = this.balance;
    moneyInTransit = 0;
    stateCaptured = true;
  ...
  private void onMoney(Money msg) {
    //real balance is always updated
    balance += msg.amount;
    

    // COMPLETED 2: implement logic for Money messages during snapshot
    //if the sender has not sent me yet the token, it means that I must save the money in the current snapshot
    if(stateCaptured && !tokensReceived.contains(getSender())){
      moneyInTransit = moneyInTransit + msg.amount;
    }
  }
  ...
  private void onToken(Token token) {
    ...
    // COMPLETED 3: manage the first Token reception and the snapshot termination for this node
    tokensReceived.add(getSender());
    //if I am not recording, start recording and send the token to everyone
    if(!stateCaptured){
      captureState();
      sendTokens();
    }
    //if I received the tokens from everyone, I stop recording the snapshot
